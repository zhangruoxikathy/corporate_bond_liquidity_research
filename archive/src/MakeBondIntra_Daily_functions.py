...
logging.basicConfig(filename=config.LOG_DIR / 'MakeBondIntra_Daily.log', encoding='utf-8', level=logging.DEBUG)

# * ************************************** */
# * Connect to WRDS                        */
# * ************************************** */
db = wrds.Connection(wrds_username=config.WRDS_USERNAME, wrds_password=config.WRDS_PASSWORD)


def pull_mergent_files():
    """ Download Mergent Files from WRDS """
    fisd_issuer = db.raw_sql("""SELECT issuer_id,country_domicile                 
                      FROM fisd.fisd_mergedissuer 
                      """)

    fisd_issuer.to_csv(config.DATA_DIR / "pulled" / "fisd_issuer_file.csv", index=False)
    ...
    fisd_issue.to_csv(config.DATA_DIR / "pulled" / "fisd_issue_file.csv", index=False)
...
def _chunk_cusips(cusips, n=500):
    """ Divide cusips into chunks of size n """
...
def _wrds_query_and_save_data(chunk, start_date, end_date):
    parm = {'cusip_id': tuple(chunk[1])}
    try:
        start = time.time()
        trace = db.raw_sql(
...
def _pull_TRACE_sequential(cusip_chunks, start_date, end_date):
    ...
    for chunk in cusip_chunks:
        partial_query_func = partial(_wrds_query_and_save_data, start_date=start_date, end_date=end_date)
        partial_query_func(chunk)


def _pull_TRACE_parallel(cusip_chunks, start_date, end_date):
...
def pull_TRACE():
    ...
    fisd = pd.read_csv(config.DATA_DIR / "pulled" / "fisd.csv")
    CUSIP_Sample = list(fisd['complete_cusip'].unique())
    cusip_chunks = _chunk_cusips(CUSIP_Sample)
    if config.RUN_TRACE_IN_PARALLEL:
        _pull_TRACE_parallel(cusip_chunks=cusip_chunks, start_date=config.START_DATE, end_date=config.END_DATE)
    else:
        _pull_TRACE_sequential(cusip_chunks=cusip_chunks, start_date=config.START_DATE, end_date=config.END_DATE)
...
def _process_clean_TRACE(file_path):
    ...
    if not match:
        raise Exception(f"Error reading file {file_path}. no chunk number.")
    ...
    if len(trace) == 0:
        ...
        logging.info(f"PID: {os.getpid()}, Pre generated no results")
        return result
    ...
    # trace = trace[(trace['days_to_sttl_ct'] == '2.0') | (trace['days_to_sttl_ct'] == '0.0') \
    #               | (trace['days_to_sttl_ct'] == '001') | (trace['days_to_sttl_ct'] == 'None')]
    trace = trace[(trace['days_to_sttl_ct'] <= 2) | (trace['days_to_sttl_ct'] == 'None')]
    ...
    if len(trace) == 0:
        logging.info(f"PID: {os.getpid()}, PostDickNielsen generated no results")
        return result
    ...
    # Price - Volume-Weight #
    trace['dollar_vol'] = (trace['entrd_vol_qt'] * trace['rptd_pr'] / 100).round(
        0)  # units x clean prc
    ...
    # =============================================================================
    logging.info(f"PID: {os.getpid()}, Total time to clean chunk {result.chunk_id}: {round(time.time() - start, 2)} secs.")
    return result
...
def clean_TRACE():
    ...
    path = config.DATA_DIR / "pulled" / "temp" / "temp_trace_[0-9]*.csv"
